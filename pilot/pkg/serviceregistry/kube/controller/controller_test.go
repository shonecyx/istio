	...
	for mode, name := range EndpointModeNames {
		...
		t.Run(name, func(t *testing.T) {
			...
			createService(controller, "svc1", "nsA",
				...
				[]int32{8080}, map[string]string{"app": "prod-app"}, t)
			fx.Wait("service")
			createService(controller, "svc2", "nsA", nil, nil, []int32{8080}, map[string]string{"app": "staging-app"}, t)
			fx.Wait("service")
...
func createService(controller *FakeController, name, namespace string,
...
func TestEndpointUpdate(t *testing.T) {
	for mode, name := range EndpointModeNames {
		...
		t.Run(name, func(t *testing.T) {
			...
			createService(controller, "svc1", "nsa", nil, nil,
				[]int32{8080}, map[string]string{"app": "prod-app"}, t)
			if ev := fx.Wait("service"); ev == nil {
				t.Fatal("Timeout creating service")
			}
	...
	for mode, name := range EndpointModeNames {
		...
		t.Run(name, func(t *testing.T) {
			...
			addService := func(name string) {
				// create service
				createService(controller, name, "nsA", nil, nil,
					[]int32{8080}, map[string]string{"app": "prod-app"}, t)
				if ev := fx.Wait("service"); ev == nil {
					t.Fatal("Timeout creating service")
				}
...
func TestTessCustom_ServiceUpdate(t *testing.T) {
	...
	// create application instance cache
	ais := []*appsv1.ApplicationInstance{
		generateApplicationInstance("foo-inst", "ns1", map[string]string{model.ApplicationServiceResourceIdLabel: "foo-app:foo-prod"}),
	}
	addApplicationInstances(t, controller, fx, ais...)

	// create service
	createService(controller, "svc1", "ns1",
		map[string]string{model.ApplicationInstanceNameLabel: "foo-inst"},
		nil,
		[]int32{8080}, map[string]string{"app": "prod-app"}, t)
	if ev := fx.Wait("service"); ev == nil {
		t.Fatal("Timeout creating service")
	...
	host := kube.ServiceHostname("svc1", "ns1", controller.domainSuffix)
	svc, _ := controller.GetService(host)
	if svc == nil {
		t.Fatal("Service not created")
	}
	if svc.Attributes.ApplicationInstance != "foo-inst" ||
		svc.Attributes.ApplicationService != "foo-app:foo-prod" {
		t.Errorf("Service expects application instance '%s' and application service '%s'\n\t"+
			"but got application instance '%s' and application service '%s'",
			"foo-inst", "foo-app:foo-prod", svc.Attributes.ApplicationInstance, svc.Attributes.ApplicationService)
	}
}
...
func TestTessCustom_ServiceUpdateBeforeApplicationInstanceUpdate(t *testing.T) {
	...
	addAi := func(name, appSvc string) {
		ai := generateApplicationInstance(name, "ns1", map[string]string{model.ApplicationServiceResourceIdLabel: appSvc})
		addApplicationInstances(t, controller, fx, ai)

	}

	deleteAi := func(name string) {
		if err := controller.tessClient.AppsV1alpha2().ApplicationInstances("ns1").Delete(context.TODO(), name, metaV1.DeleteOptions{}); err != nil {
			t.Fatal(err)
		}
		retry.UntilSuccessOrFail(t, func() error {
			controller.appInsts.RLock()
			defer controller.appInsts.RUnlock()
			key := kube.KeyFunc(name, "ns1")
			if _, ok := controller.appInsts.getApplicationService(key); ok {
				return fmt.Errorf("application instance still present")
			}
			return nil
		}, retry.Timeout(time.Second))
	}

	addService := func(name string, appInstName string) {
		createService(controller, name, "ns1",
			map[string]string{model.ApplicationInstanceNameLabel: appInstName},
			nil,
			[]int32{8080}, map[string]string{"app": "prod-app"}, t)
		if ev := fx.Wait("service"); ev == nil {
			t.Fatal("Timeout creating service")
	...
	assertServiceEvent := func() {
		t.Helper()
		ev := fx.Wait("service")
		if ev == nil {
			t.Fatalf("Timeout wait service")
		}
	}

	assertServiceAttribute := func(name, appInst, appSvc string) {
		t.Helper()
		host := kube.ServiceHostname(name, "ns1", controller.domainSuffix)
		svc, _ := controller.GetService(host)
		if svc == nil {
			t.Fatalf("Service %s not created", host)
		}
		if svc.Attributes.ApplicationInstance != appInst ||
			svc.Attributes.ApplicationService != appSvc {
			t.Errorf("Service expects application instance '%s' and application service '%s'\n\t"+
				"but got application instance '%s' and application service '%s'",
				appInst, appSvc, svc.Attributes.ApplicationInstance, svc.Attributes.ApplicationService)
		}
	}
	...
	assertPendingResync := func(expected int) {
		...
		retry.UntilSuccessOrFail(t, func() error {
			controller.appInsts.RLock()
			defer controller.appInsts.RUnlock()
			if len(controller.appInsts.needResync) != expected {
				return fmt.Errorf("expected %d application instance needing resync, got %d",
					expected, len(controller.appInsts.needResync))
			}
			return nil
		}, retry.Timeout(time.Second))
	}

	// service update comes first
	assertPendingResync(0)
	addService("foo-svc", "foo-inst")
	assertServiceAttribute("foo-svc", "foo-inst", "")
	assertPendingResync(1)

	// application instance update comes next
	addAi("foo-inst", "foo-app:foo-prod")
	assertServiceEvent()
	assertServiceAttribute("foo-svc", "foo-inst", "foo-app:foo-prod")
	assertPendingResync(0)
	fx.Clear()

	// deleting application instance doesn't cause service got updated
	// that's, service attribute still has the old application service
	deleteAi("foo-inst")
	assertPendingResync(0)
	assertServiceAttribute("foo-svc", "foo-inst", "foo-app:foo-prod")
	fx.Clear()
